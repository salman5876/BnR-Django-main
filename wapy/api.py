...
API_BASE_PAGINATION='http://api.walmartlabs.com'
...
class Wapy:
    ...
    def pagination(self,categoryId,nextpage,**kwargs):

        if nextpage != None:
            if type(nextpage) == int:
                if type(categoryId) == str:
                    url = API_BASE_URL+'paginated/items?category='+categoryId+'&maxId='+str(nextpage)
                else:
                    raise InvalidParameterException('categoryId should be a String value')
            else:
                raise InvalidParameterException('nextpage should be a integer value')
        else:
            return [],None

        '''
        else:
            url = API_BASE_PAGINATION+nextpage
        '''
        ...
        if len(data)>0:
            ...
            if len(data["items"]) > 0:
                ...
                return products, int(re.search('maxId=(\d+)',data["nextPage"]).group(1))
            else:
    ...
    def search(self, query, **kwargs):
        ...
        if data["totalResults"]>0:
            for item in data["items"]:
                products.append(WalmartProduct(item, self.LSNID))
            return products,data["totalResults"]
        else:
            return products
