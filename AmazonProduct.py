...
from products.models import AmazonURLs, AmazonProxies, AllProducts
...
class Products(ScrapeTools):
    textFile = 'Amazon_min_max_range.txt'
    def __init__(self):
    ...
    def get_data_Textfile(self):
        ...
        with open(self.textFile, 'r') as f:
            min_range = f.readline().strip()
            max_range = f.readline().strip()
        data = self.getScrapeData(min_range=min_range,max_range=max_range)

        for item in data:
            data1.append((item['Cat'],item['SubCat'],item['url']))
        # self.changeStatus(data1[0])
        # with open(self.textFile, 'r') as f:
        #
    ...
    def scrape_data(self,soup, url):
        ...
        while is_next:
            ...
            for current_item_div in items_div:
                ...
                try:
                    try:
                        pro_check = "AZ" + current_item_div.parent["data-asin"]
                    except:
                        pro_check = "AZ" + current_item_div["data-asin"]
                    if AllProducts.objects.filter(SNR_SKU=pro_check,SNR_Available='Amazon').exists() == False:
                        ...
                        try:
                            ...
                            linkkk = current_item_div.select_one("span.rush-component a.a-link-normal")["href"]
                            pro_url = self.getAbsUrl(url, linkkk)
                        ...
                        try:
                            ...
                            try:
                                Price = soup_pro.find('span', {'id': 'priceblock_ourprice'}).text.split('-', 1)[0].split('$',1)[1].strip()
                                Price = [i.replace(',', '') for i in Price]
                                Price = float(''.join(Price))
                            except:
                                try:
    ...
    def Data_Scraper(self,data):
        ...
        # obj = self.scrape_data(post_soup, url_link)
        # for o in obj:
