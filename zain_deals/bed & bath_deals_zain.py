...
class BedSpider(scrapy.Spider):
    ...
    def product_data(self,response):
        ...
        try:
            ...
            if not price:
                price = float(response.xpath('//*[@itemprop="price"]/text()').extract_first())
                price = price*0.0086
                request["SNR_Price"] = price
            else:
                price = float(response.xpath('//*[@itemprop="lowPrice"]/text()').extract_first())
                price = price*0.0086
                request["SNR_Price"] = price
        except Exception:
            pass
        try:
            ...
            if not price_before:
                ...
                price_before = price_before*0.0086
