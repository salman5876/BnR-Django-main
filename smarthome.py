...
def smarthome_Walmart(request,categoryId,nextpage=0):
        ...
        for product in products:
            ...
            if (product.available_online == True) and (product.sale_price != None):
                ...
                if product.model_number != None and laptops[product.model_number] == None:
                    ...
                    request["SNR_SubCategory"] = "Smart Home"
...
class EbayAPI():
    ...
    def ebaySmartHomeAll(self, request):
        ...
        for category in listCatagories:
            response = requests.get(
                "http://svcs.ebay.com/MerchandisingService?OPERATION-NAME=getRelatedCategoryItems&SERVICE-NAME=MerchandisingService&SERVICE-VERSION=1.1.0&CONSUMER-ID=huzaifaz-ERecomme-PRD-008fa563f-8f5d310e&RESPONSE-DATA-FORMAT=JSON&REST-PAYLOAD&maxResults=1000&categoryId="+category)
...
EbayAPI().ebaySmartHomeAll({})


listCatagories = ['1072864_1229875']
for category in listCatagories:
    time.sleep(30)

    smarthome_Walmart({}, category)

GroupOnAPI().groupHome({})
time.sleep(30)
# newEggAPI().newEgggGames({})
EbayAPI().ebaySmartHomeAll({})
time.sleep(30)

bestbuy().getSmartHomes({})
time.sleep(30)
...
class MyCronJob(CronJobBase):
    ...
    code = 'smarthomes.my_cron_job'    # a unique code
